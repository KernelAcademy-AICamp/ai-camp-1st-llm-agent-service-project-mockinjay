...
export async function fetchClinicalTrials(
  ...
  try {
    const response = await api.post('/api/clinical-trials/list', {
      condition,
      page,
      page_size: pageSize,
    });
...
export async function fetchNews(
  ...
  try {
    const response = await api.post('/api/news/list', {
      query,
      language,
      page: 1,
      page_size: 10,
    ...
    return articles.map((article: Record<string, unknown>) => ({
      id: article.id ?? article.link ?? article.url ?? `${article.title}-${Date.now()}`,
      title: article.title ?? '',
      ...
      publishedAt: article.publishedAt ?? article.pubDate ?? article.time ?? '',
      language: (article.language as 'en' | 'ko') ?? language,
    }));
  } catch (error) {
