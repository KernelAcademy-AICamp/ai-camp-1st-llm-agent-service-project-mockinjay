...
class Dataset8Loader:
    """
    Loader for AI Hub Dataset 8 - 전문 의학지식 데이터
    ...
    def __init__(self, base_path: str):
        """
        Initialize the Dataset 8 loader

        Args:
            base_path: Base path to the dataset 8 directory
        """
        self.base_path = Path(base_path)
        self.dataset_name = "dataset_8_professional_medical"
    ...
    def _find_json_files(self) -> List[Path]:
        ...
        logger.info(f"Found {len(json_files)} JSON files in dataset 8")
        return json_files
    ...
    def load(self) -> List[Dict]:
        """
        Load all QA pairs from dataset 8
        ...
        logger.info(f"Loaded {len(qa_pairs)} QA pairs from dataset 8")
        return qa_pairs
...
def load_dataset_8(base_path: str) -> List[Dict]:
    """
    Convenience function to load dataset 8
    ...
    Returns:
        List of QA pairs
    """
    loader = Dataset8Loader(base_path)
    return loader.load()
...
if __name__ == '__main__':
    ...
    base_path = "data/ai_hub/08.전문 의학지식 데이터"
    loader = Dataset8Loader(base_path)
